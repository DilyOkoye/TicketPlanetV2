...
+ "&orderId=" + orderId + "&amount=" + amount;
        ...
        public List<tk_Cinema> GetCinemaLocations(int cinemaID) 
        {
            if (cinemaID == 1)
            {
                return repoCinema.GetManyNonAsync(o => o.Status == "Active" && o.CinemaName.Contains("Genesis Delux") && o.SiteId == null).ToList();
        ...
        public int CalculateFlutterAmount(string Amount, int NoOfPerson, string CouponValue)
        {
            string amt = Amount.Replace(",", "");
            decimal amount = 0.0m;
            if (decimal.TryParse(amt, out amount))
